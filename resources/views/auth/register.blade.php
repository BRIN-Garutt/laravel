<x-authentication-layout>
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="space-y-4">
            <div>
                <x-label for="name" class="text-white">{{ __('Full Name') }} <span class="text-yellow-500">*</span></x-label>
                <x-input id="name" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
            </div>

            <div>
                <x-label for="email" class="text-white">{{ __('Email Address') }} <span class="text-yellow-500">*</span></x-label>
        ...
        <div class="flex items-center justify-between mt-6">
            <div class="mr-1">
                <label class="flex items-center" name="newsletter" id="newsletter">
                    ...
                    <span class="text-sm ml-2 text-white">Email me about product news.</span>
                </label>
            </div>
            <x-button>
                {{ __('Sign Up') }}
    ...
    <div class="pt-5 mt-6 border-t border-gray-100 dark:border-gray-700/60">
        <div class="text-sm text-white">
            {{ __('Have an account?') }} <a class="font-medium text-yellow-500 hover:text-yellow-600 dark:hover:text-yellow-400" href="{{ route('login') }}">{{ __('Sign In') }}</a>
        </div>
    </div>
</x-authentication-layout>