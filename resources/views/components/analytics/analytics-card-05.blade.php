...
<head>
    ...
    <title>Real Time Kelembapan Pameungpeuk Garut</title>
...
<body class="dark">
    ...
    <div class="flex flex-col col-span-full sm:col-span-6 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
        <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex items-center">
            <h2 class="font-semibold text-gray-800 dark:text-gray-100">Kelembapan Pameungpeuk Garut</h2>
        ...
        <div class="px-5 py-3">
            <div class="flex items-start">
                <div class="text-3xl font-bold text-gray-800 dark:text-gray-100 mr-2 tabular-nums"><span id="humidity-value">--</span>%</div>
                <div id="humidity-deviation" class="text-sm font-medium px-1.5 rounded-full"></div>
            </div>
        </div>
        <div class="grow">
            <canvas id="humidity-chart" width="595" height="248"></canvas>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function fetchRealTimeHumidity() {
                fetch(apiUrl)
                    ...
                    .then(data => {
                        ...
                        // document.getElementById('humidity-deviation').textContent = data.deviation;
                    })
                    .catch(error => console.error('Error fetching real-time humidity:', error));
            }

            // Initialize humidity fetch
            fetchRealTimeHumidity();

            // Fetch humidity every 1 seconds
            setInterval(fetchRealTimeHumidity, 1000);
